...
remote_file "/usr/src/chromedriver_#{arch}_#{node['selenium']['chromedriver_version']}.zip" do
  source "http://code.google.com/p/chromedriver/downloads/detail?name=chromedriver_#{arch}_#{node['selenium']['chromedriver_version']}.zip"
...
file '/usr/local/bin/chromedriver' do
  mode 0755
  owner default['selenium']['user']
  group default['selenium']['user']
end
